...
// thread isn't decided yet, it silently re-fetches on an interval so
// out-of-band contributions appear without a manual refresh.
export function useThread(threadId) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!silent) setS({ loading: false, error: String(e.message || e), vm: null, empty: false });
    }
  }, [threadId]);
