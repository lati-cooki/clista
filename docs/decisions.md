...
| **Canonical source designation** — `clista-protocol` sole authority for protocol facts; atlas explicitly derivative (navigation, never facts). Option B proposed in the DR; five binding rules: supremacy clause in atlas README, per-page provenance pins (repo + commit hash), same-commit rule scoped to within-atlas, single external entry point via `clista-protocol`, CI pin validation (rule 5, added by pre-seal amendment seq 7 after hermes-raft's challenge `obj_supremacy-enforceability` seq 4) | ThreadHub thread `canonical-source-designation` (`thd_c1a2a74df65b`), owner `id_troy`; DR = seq 1, amendment = seq 7 `sha256:213140184fc34875ad7767664320937bb70b2121db425af260eed02be0726cf2` (local copy: [decisions/DR-2026-07-08-canonical-source.md](decisions/DR-2026-07-08-canonical-source.md)); hermes-raft challenged (seq 2–6, supports with caveats); Clista stood down — its runtime turned out to execute on TheMacLati.local (no hub access; RESOLVED 2026-07-08 evening: recreated as @ClisTagent on TheLatiMac.local and can rejoin; the second challenge pass now falls to @ProtocolCodex — see identities.md) | **Proposed, pending owner seal**. ProtocolCodex second pass DONE (seq 8–11): supports amended Option B, objection `obj_seal-must-separate-policy-from-implementation` (seq 10) — don't imply the controls are live before they exist. **Rules 1–5 IMPLEMENTED 2026-07-08 late** in direct response: supremacy clause (atlas README), per-page provenance pins (all pages), same-commit rule scoped (README), entry-point section (clista-protocol README @ `d76bd566`, local commit pending push), CI pin validation (`scripts/validate-pins.mjs` + `.github/workflows/pin-validation.yml`, passing). Remaining for seal: owner record on the hub thread citing the implementation commits; then cite the seal hash here |
