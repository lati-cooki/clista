...
| **Canonical source designation** — `clista-protocol` sole authority for protocol facts; atlas explicitly derivative (navigation, never facts). Option B accepted in the DR; four binding rules: supremacy clause in atlas README, per-page provenance pins (repo + commit hash), same-commit rule scoped to within-atlas, single external entry point via `clista-protocol` | DR staged at [decisions/DR-2026-07-08-canonical-source.md](decisions/DR-2026-07-08-canonical-source.md), owner `troy_builds` | **Proposed, pending seal** as a signed ThreadHub event. On adoption: implement rules 1–4 across the atlas and cite the seal's record hash here |
