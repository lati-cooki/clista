...
| **Canonical source designation** — `clista-protocol` sole authority for protocol facts; atlas explicitly derivative (navigation, never facts). Option B proposed in the DR; four binding rules: supremacy clause in atlas README, per-page provenance pins (repo + commit hash), same-commit rule scoped to within-atlas, single external entry point via `clista-protocol` | ThreadHub thread `canonical-source-designation` (`thd_c1a2a74df65b`), owner `id_troy`; DR staged as record seq 1, hash `sha256:154f35779c4c4a7869350692674336fc7546da3688d5f5598cf332211c993269` (local copy: [decisions/DR-2026-07-08-canonical-source.md](decisions/DR-2026-07-08-canonical-source.md)) | **Proposed, pending owner seal** on the hub thread. On adoption: implement rules 1–4 across the atlas and cite the seal record's hash here |
